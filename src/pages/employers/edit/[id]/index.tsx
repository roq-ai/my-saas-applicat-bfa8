...
import { getEmployerById, updateEmployerById } from 'apiSdk/employers';
...
function EmployerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EmployerInterface>(
    () => (id ? `/employers/${id}` : null),
    () => getEmployerById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: EmployerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateEmployerById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<EmployerInterface>({
    initialValues: data,
