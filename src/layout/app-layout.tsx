...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'employee-with-full-access'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<EmployerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/employers` : null),
    () => getEmployers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Employer',
      path: isTenantUser ? `/employers/view/${data?.[0]?.id}` : '/employers',
      entity: 'employer',
